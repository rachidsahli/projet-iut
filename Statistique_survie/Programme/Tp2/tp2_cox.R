...
fit3 = coxph(Surv(churn$tenure, churn$Churn) ~ factor(churn$Dependents))
summary(fit3)

### Interpretation de fit3 :

fit4 = coxph(Surv(churn$tenure, churn$Churn) 
             ~ factor(churn$Dependents) 
             + factor(churn$Partner))
summary(fit4)

### Interpretation de fit4 :


fit5 = coxph(Surv(churn$tenure, churn$Churn) 
             ~ factor(churn$Dependents) 
             + factor(churn$Partner) 
             + factor(churn$Contract))
summary(fit5)

### Interpretation de fit5 :


fit6 = coxph(Surv(churn$tenure, churn$Churn) 
             ~ factor(churn$Dependents) 
             + factor(churn$Partner) 
             + factor(churn$Contract)
             + factor(churn$InternetService))
summary(fit6)

### Interpretation de fit6 : 
